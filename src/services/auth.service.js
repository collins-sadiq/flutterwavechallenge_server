...
class AuthService {
  async signup(data) {
    ...
    //get dispatch rider to assigned for the merchant
    const rider = await Rider.findOne({ assigned: false });

    //create flutterwave sub account for the merchant
    const result = await createSubAccount(data);
    
    if (user) throw new CustomError("Email already exists");

    if (result.status === "error") throw new CustomError(result.message);
